...
export class JsonValidator {
            ...
            files.forEach(file => {
                if (extname(file) === '.json') {
                    ...
                    try {
                        ...
                        throw new Error(`Invalid JSON in schema file: ${file}\n${e && (e.message || String(e))}`);
                    }
                    try {
                        this.ajv.compile(schema); // validate schema itself
                    } catch (e: any) {
                        throw new Error(`Invalid JSON Schema in file: ${file}\n${e && (e.message || String(e))}`);
                    }
    ...
    public serializeJsonWithSchema<T>(jsonData: unknown, schemaPath: string): T {
        ...
        if (!validate) {
            throw new Error(`Schema not found: ${schemaKey} (while validating file: ${schemaPath})`);
        }
        let valid: boolean = false;
        let sourceMap: any = undefined;
        let originalText: string | undefined = undefined;
        // Try to get line numbers if jsonData is a plain object from JSON.parse
        if (typeof jsonData === 'object' && jsonData !== null && (jsonData as any).__sourceMapText) {
        ...
        try {
            ...
            throw new Error(`Validation error in file '${schemaPath}': ${err && (err.message || String(err))}`);
        }
        if (!valid) {
            ...
            if (validate.errors && originalText && sourceMap) {
                errorDetails = validate.errors.map((e: any) => {
                    const pointer = sourceMap.pointers[e.instancePath || ''];
                    const line = pointer ? pointer.key ? pointer.key.line + 1 : pointer.value.line + 1 : undefined;
     ...
     */
    public serializeJsonFileWithSchema<T>(filePath: string, schemaPath: string): T {
        let fileText: string;
        let data: unknown;
        let pointers: any;
        try {
        ...
        try {
            ...
            const err = new Error(`Validation failed for file: ${filePath}\n${e && (e.message || String(e))}`);
